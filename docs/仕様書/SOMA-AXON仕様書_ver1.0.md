...
AXONとSOMA間の通信フレームを下記のTable 3-4に示します。基本的なフレームフォーマットは、下記のTable 3-4の通り、ヘッダ1 Byteと、データ32 Byte、CRC16 2Byte、の合計36バイトで構成されます。
...
| CHK_TOUT | 18th | [0:7] | ・電子マネー用 ソレノイドON時間のタイムアウト設定確認<br><br>**設定値とタイムアウト時間**:<br>**[7:4]:** ・RFU("0"固定)<br><br>**[3:0]:**<br>0x0: 30秒 (Default)<br>0x1: 15秒<br>0x2: 20秒<br>0x3: 25秒<br>0x4: 30秒<br>0x5: 35秒<br>0x6: 40秒<br>0x7: 45秒<br>0x8: 50秒<br>0x9: 55秒<br>0xA: 60秒<br>0xB: 90秒<br>0xC: 120秒<br>0xD: 150秒<br>0xE: 無限秒<br>上記以外: 30秒 (異常値) |
...
SOMA-AXON間では、AES256による暗号化通信が行われます。
AES256の運用鍵は、SOMA基板からAXON基板に対して、運用鍵更新コマンド（SETOKEY）を送信する事で更新されます。
運用鍵更新コマンドは、16バイトの新しい運用鍵を含んでいます。
AXON基板は、運用鍵更新コマンドを受信し、新しい運用鍵にて動作を開始します。
受信に成功した場合、AXON基板は新しい運用鍵にて動作を開始し、SOMA基板に対してコマンドACK応答を送信します。
受信に失敗した場合、AXON基板は旧運用鍵のまま動作を継続し、SOMA基板に対してコマンドNAK応答を送信します。

SOMA-AXON間では、チャレンジ/レスポンスパケットを使用した認証処理は行われません。
...
sequenceDiagram
    ...
    Note over SOMA: 新運用鍵（16バイト）を準備（TG-SOMA間で取得済み）

    SOMA->>AXON: [SETOKEY] 運用鍵更新コマンド<br>(Header: 0x15, 新運用鍵16バイト)

    alt 受信成功
        Note over AXON: 新運用鍵を保存<br>新運用鍵で動作開始
        AXON-->>SOMA: [ACK] 更新成功
        Note over SOMA: 新運用鍵で通信継続
        Note over AXON: 新運用鍵で通信継続
    else 受信失敗
        Note over AXON: 旧運用鍵のまま動作継続
        AXON-->>SOMA: [NACK] 更新失敗
        Note over SOMA: 旧運用鍵で通信継続<br>リトライ可能
    end
```

**注意事項:**
- TG-SOMA間とは異なり、チャレンジ/レスポンスパケットによる認証は不要
- 単一のSETOKEYコマンドで運用鍵を直接更新
- ACK/NACK応答で更新の成否を確認
